            ...
            messages=[
                {
                    "role": "user",
                    "content": [
                        {"type": "text", "text": "請作為嚴謹的影像鑑識助理，分析我給的圖片：逐字轉錄所有文字；所有數字讀兩遍並交叉比對，用表格列出與置信度。列出畫面元素、數量、位置、色彩與佈局；標註任何不確定處。凡涉及計算，一律用數學工具計算並展示步驟。在本次對話期間記住所有細節供後續提問。依「摘要→轉錄→數字表→元素→版面→線索→計算→不確定性→待驗證」的順序輸出。"},
